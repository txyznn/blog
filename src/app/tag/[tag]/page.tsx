...
export function generateStaticParams() {
  return getAllTags().map((term) => ({tag: term.name}));
}
...
export async function generateMetadata({
  ...
  return {
    title: `${tag} | Chaowen`,
    description: `Chaowen Notes 中带有「${tag}」标签的全部文章。`,
  };
}
  ...
  return (
    <main className="flex min-h-screen flex-col bg-background">
      ...
      <section className="mx-auto w-full max-w-4xl px-5 pb-20 pt-12 md:px-8 md:pt-16">
        ...
        <nav className="mt-8 flex flex-wrap gap-2" aria-label="全部标签">
          {tags.map((term) => (
            <Badge
              key={term.name}
              asChild
              variant={term.name === tag ? 'default' : 'outline'}
